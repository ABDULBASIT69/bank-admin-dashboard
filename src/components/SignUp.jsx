...
export default function SignUp() {
   ...
   let details ={}
    const registration = async (data) =>{
        ...
        console.log('metadata',storageRef)
        ...
        (snapShot) => {
            ...
            getDownloadURL(uploadTask.snapshot.ref)
             .then(fireBaseUrl => {
                    addDoc(collection(db,'user-registration'),{
                ...
                <div className="container mx-auto px-6">
                    ...
                    <form action="" onSubmit={handleSubmit(registration)}>
                        <div className="pt-5">
                            ...
                            <div className="input-outline py-5 px-7 my-4"><input type="text" name="fullname" id="fullname" {...register('fullName',{required:true})} placeholder="Full Name" className="w-full" /></div>
                            {errors.fullName && <span className='error-color'>Enter Your Full Name</span>}
                        </div>
                        <div>
                            <span className="gray">Email Address</span>
                            <div className="input-outline py-5 px-7 my-4"><input type="email" name="email" id="email" {...register('email',{required:true})} placeholder="Email Address" className="w-full" /></div>
                        ...
                        <div className="full-fields flex justify-between">
                            <div className='w-full mr-5'>
                                ...
                                <div className="input-outline py-5 px-7 my-4"><input type="date" name="birthdate" id="birthdate" {...register('birthDate',{required:true,valueAsDate: true,})} placeholder="Birth Date" className="w-full" /></div>
                                {errors.birthDate && <span className='error-color'>Enter Your Birth Date</span>}
                            </div>
                            <div className='w-full ml-5'>
                                <span className="gray">Phone Number</span>
                                <div className="input-outline py-5 px-7 my-4"><input type="number" name="phonenumber" id="phonenumber" {...register('phoneNumber',{required:true})} placeholder="Phone Number" className="w-full"  /></div>
                        ...
                        <div className="full-fields flex justify-between">
                            <div className='w-full mr-5'>
                                ...
                                <div className="input-outline py-5 px-7 my-4"><input type="text" name="province" id="province" {...register('province',{required:true})} placeholder="Province" className="w-full"  /></div>
                                {errors.province && <span className='error-color'>Enter Your Province Name</span>}
                            </div>
                            <div className='w-full ml-5'>
                                <span className="gray">City</span>
                                <div className="input-outline py-5 px-7 my-4"><input type="text" name="city" id="city" {...register('city',{required:true})} placeholder="City" className="w-full"  /></div>
                                {errors.city && <span className='error-color'>Enter Your City Name</span>}
                            </div>
                        </div>
                        <div>
                            <span className="gray">Full Address</span>
                            <div className="input-outline py-5 px-7 my-4"><textarea name='fullAddress' id='fullAddress' {...register('fullAddress',{required:true})} placeholder="Full Address" cols="12" rows="2" className="w-full"  /></div>
                            {errors.fullAddress && <span className='error-color'>Enter Your Full Address</span>}
                        </div>
                        <div>
                            <span className="gray">Profile Picture</span>
                            <div className="input-outline py-5 px-7 my-4"><input type="file" name="profileImage" id="profileImage" {...register('profileImage',{required:true})} placeholder="profileImage" className="w-full"  /></div>
                            {errors.profileImage && <span className='error-color'>Select Your Profile Image</span>}
                        </div>
                        <div className="full-fields flex justify-between">
                            <div className='w-full mr-5'>
                                <span className="gray">Password</span>
                                <div className="input-outline py-5 px-7 my-4"><input type="password" name="password" id="password" {...register('password',{required:true})} placeholder="Password" className="w-full"  /></div>
                                {errors.password && <span className='error-color'>Enter Your Password</span>}
                            </div>
                            <div className='w-full ml-5'>
                                <span className="gray">Confirm Password</span>
                                <div className="input-outline py-5 px-7 my-4"><input type="password" name="confirmPassword" id="confirmPassword" {...register('confirmPassword',{required:true,validate:(val)=>{
